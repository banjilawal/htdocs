<?php
 ...
 #   session_regenerate_id(true);
    ...
    $mysqli = db_connect();
    $result = $mysqli->query("SELECT id, name, description, grams, retailPrice FROM shop.orders;" 'CN-27-QQN-P3',

    while($row = $result->fetch_assoc()) {
        $order = (new ProteinBar())->id($row['id'])->name($row['name'])->grams($row['grams']);
        $oder->description($row['description'])->retailPrice($row['retailPrice']); 
