<?php
 ...
 #   session_regenerate_id(true);
    ...
    $mysqli = db_connect();
    $result = $mysqli->query("SELECT id, customerID, status, submissionDate,  estimatedDeliveryDate DATETIME, # AS (DATE_ADD(submissionDate, INTERVAL 5 DAY)),
    actualDeliveryDate DATETIME,
    total DECIMAL (6,2),FROM shop.orders");

    while($row = $result->fetch_assoc()) {
        $order = (new order())->id($row['id'])->name($row['name'])->grams($row['grams']);
