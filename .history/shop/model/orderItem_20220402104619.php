    ...
    class OrderItemBag {
        ...
        public function add ($orderItem) {
            if (get_class($orderItem) != get_class(new OrderItem())) {
                throw new Exception("Inavlid class parameter for OrderItemBag->add()");
            }
        ...
        public function search ($item) {
            if (get_class($item) != get_class((new OrderItem ()))) {
                $target = $item->get_protein_bar()->get_name();
            }
            else if (get_class($item) != get_class((new ProteinBar ()))) {
                $target = $item->get_name();
            }
            else  {
            ...
            foreach ($this->list as $orderItem) {
                if ($orderItem->get_protein_bar()->get_name() == $target) {
                    $result = $orderItem;
                    break;
                }
            }
