<?php
    DEFINE ('CREDIT_CARD_PATTERN', '/([0-9]{3,}-){3,}/i');
    DEFINE ('CARD_STATUS', array('chargable', 'expired', 'declined'));
    class CreditCard {
        private $number;
        ...
        public function status ($status) {
            if (in_array($this->status, CARD_STATUS, $strict = true) == false) {
                throw new Exception($t . " is not a valid state acronym");
            }
            $this->state = $state;
            return $this;
        }
        ...
        public function __toString() {
            $text = 'card: **-' . $this->print_number() 
                . ' expiration: '. $this->date_string()
