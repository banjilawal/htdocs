<?php
    ...
    $card = new CreditCard();
    $card->number('8440-51076-0887')->expiration((new DateTime('2022-06-15')))->security_code('237');

    $phone = new Phone();
    $phone->area_code('340')->exchange(679)->number(8013);
    
    $customer = new Customer();
    $customer->firstname('Hannia')->lastname('Free')->birthdate((new DateTime('1976-06-17')))->phone($phone)->address($address)->card($card)->email('hannia-free@aim.com');
    echo '<p>' . $customer . '</p>';

    echo '<p>' . $customer->to_table() . '</';
    #*/
