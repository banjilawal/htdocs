<?php
    class Phone {
        ...
        public function string_to_phone ($text) {
            $this->areaCode = substr($text, (strpos($text,'(') + 1 ),  (strpos($text,')') - 1 ) );
            $this->exchange = substr($text, (strpos($text,' ') + 1 ),  (strpos($text,')') - 1 ) );
            $this->number = substr($text, (strpos($text,'-') + 1 ) );
        }
        ...
        public function is_equal ($object) {
            $result 
        } 
...
/*   
    $p = new Phone();
    $p->string_to_phone('(612) 284-6795');
    echo $p;
*/
/*    
    $phone = new Phone();
