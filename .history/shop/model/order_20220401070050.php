<?php
    ...
    DEFINE ('ORDER_ID_PATTERN', '');
    DEFINE ('ORDER_STATES', array());

    CONSTRAINT order_id_ck CHECK (id REGEXP '[A-Z][A-Z][A-Z][A-Z][A-Z]-[0-9][0-9][0-9][0-9][0-9]-[A-Z][A-Z][A-Z][A-Z]-[2-9][2-9][2-9]'),
    CONSTRAINT order_status_ck CHECK (status IN ('cancelled', 'returned', 'in transit', 'delivery verified', 'delivery not confirmed')),
    ...
    class Order {
        ...
        public function status ($status) {
            if (in_array($this->status, CARD_STATUS, $strict = true) == false) {
