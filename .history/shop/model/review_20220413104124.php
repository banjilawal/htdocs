...
*/
        ...
        public function review_by_customer_to_table () {
            if (is_null($this->customerID)) {
                throw new Exception ("Cannot show customer reviews when customerID is null for reviewBag");
            }
                ...
                . '<tbody>';
                    foreach ($this->list as $id => $review) {
                        $elem = '<tr id=">' .$id . '" onclick="send_review(this)">'
                            . '<td>' . $this->list[$id]->get_stars() . '</td>'
                            . '<td><img src="' . $this->list[$id]->get_image_path() . '" width="90" height="100"></td>'
        ...
        public function review_by_bar_to_table () {
            if (is_null($this->proteinBarID)) {
                throw new Exception ("Cannot show reviews by proteinbar when proteinbarID is null for reviewBag");
            }
            else if (is_null($this->proteinBarID) == false) {
            ...
            $elem = '<table class>'
                . '<thead>'
                    . '<tr>'
                        . '<th>Stars</th>'
                        . '<th>Customer First Name Last </th>'
                        . '<th>Review Date</th>'
                        . '<th>Comments</th>'
                    . '</tr>'  
                . '</thead>'
                . '<tbody>';
                    ...
                    if (is_null($this->customerID) == false) {
                        foreach ($this->list as $id => $review) {
                            $elem = '<tr id=">' .$id . '" onclick="send_review(this)">'
                                . '<td>' . $this->list[$id]->get_stars() . '</td>'
                                . '<td><img src="' . $this->list[$id]->get_image_path() . '" width="090" height="100"></td>'
                                . '<td>' . $this->list[$id]->get_protein_bar_name() . '</td>'
                                . '<td>' . $this->list[$id]->get_timestamp()->format('Y-m-d hh:mm:ss') . '</td>'
                                . '<td>' . $this->list[$id]->get_comments() . '</td>'
                            . '</tr>';             
