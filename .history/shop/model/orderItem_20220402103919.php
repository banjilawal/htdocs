<?php
    ...
    class OrderItemBag {
        ...
        public function add ($orderItem) {
            ...
            $index = $this->index_of_name($barName);
            
            if ( $index > -1 ) {
                $this->list[$index]->increase_quantity(1);
            } 
            else {
                $this->list[] = $orderItem;
            }
        ...
        public function search ($target) {
            if (get_class($target) != get_class((new OrderItem ()))) {
                $targetName = $target->get_protein_bar()->get_name();
            }
            else if (get_class($target) != get_class((new ProteinBar ()))) {
                $targetName = $target->ge;
            }
            else (get_class($target) != get_class((new ProteinBar ()))) {
