<?php
    class Phone {
        ...
        public function string_to_phone ($text) {
            $this->areaCode = substr($text, strpos($text,'('),  strpos($text,'(') );
            $this->exchange = substr($text, strpos($text,')'),  strpos($text,'-') );
            $this->number = substr($text, strpos($text,'('),  strpos($text,'(') );
            return  substr($text, strpos($text,'('),  strpos($text,'(') );
    ...
    $p = new Phone();
    echo $p->string_to_phone('(612) 284-6795');
/*    
$phone = new Phone();
