<?php
    ...
    $bar->description('thigh,broilers or fryers,meat and skn,ckd,chicken,stwd, crackers (made with whl grain),cheez-it,suns');
    $bars->add($bar);

    $bar = new ProteinBar();
    $bar->id('PN-KRV-363-T')->name('Fleisher')->grams(48)->retailPrice(2.99);
    $bar->description('Saraban', 'oat bran,bagels, sauce,canned,w/onions,tomato products, unpr,frz,kashi three cheese ravioli with med');
    $bars->add($bar);

    $bar = new ProteinBar();
    $bar->id('PN-XTP-653-P')->name('Rochberg')->grams(95)->retailPrice(1.99);
    $bar->description('m and ms crispy choc c and ies,mars snackfood us,c and ies, apl),grp,ocean spry,w/vit c,ruby red grp',);
    $bars->add($bar);

    $bar = new ProteinBar();
    $bar->id('PN-KRV-363-T')->name('Fleisher')->grams(48)->retailPrice(2.99);
    $bar->description('minis multigrain crackers,club,keebler, frozen,lasagna with meat and sauce, loin,frsh,country-style');$bars->add($bar);
