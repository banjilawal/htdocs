<?php
    ...
    function customer_orders_query ($customerID) {
        ...
        $query = 'SELECT o.id orderID, submissionDate, estimatedDeliveryDate, actualDeliveryDate, '
            . 'status, total FROM shop.orders o INNER JOIN shop.customers c '
        ...
        $stmt->bind_result($id, $submissionDate, $estimatedDeliveryDate, $actualDeliveryDate, $status, $total);
        ...
        while ($stmt->fetch()) {
            ...
            $order->add_orderItem($orderId, $orderItem);
            #$order->submission_date(new DateTime($submissionTime));
