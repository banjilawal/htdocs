<?php
    ...
    function insert_customer_account ($customer, $password) {
        ...
        $account = $prefix = substr($email, 0, strrpos($email, '@'));
        $query = 'INSERT INTO shop.customer_accounts (username, pass) VALUES (?, ?)';

        $account_insert_stmt = $mysqli->prepare($query);
        $account_insert_stmt->bind_param('ssssssssss', $customerID, $firstname, $lastname, $birthdate, $phone, $email, $street, $city, $state, $zip);
        $stmt->execute();  
