<?php
    ...
    function insert_customer ($customerID, $firstname, $lastname, $birthdate, $phone, $email, $street, $city, $state, $zip) {
        $mysqli = db_connect();
        $customerID = assign_customer_id();

       # echo 'customerID: ' . $customerID . ' cardID: ' . $cardID . ' expiration: ' . $expiration . ' code: ' . $securityCode . '<br>';
        $query = 'INSERT INTO shop.customers (id, firstname, lastname, birthdate, phone, email, street, city, tate, zip) '
            . 'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';

        $stmt = $mysqli->prepare($query);
        $stmt->bind_param('sss sss sss s', $customerID, $firstname, $lastname, $birthdate, $phone, $email, $street, $city, $state, $zip);
        $stmt->execute();   
