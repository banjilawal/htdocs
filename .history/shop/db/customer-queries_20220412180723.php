<?php
    ...
    function insert_customer ($firstname, $lastname, $birthdate, $phone, $email, $street, $city, $state, $zip) {
       ...
       # echo 'customerID: ' . $customerID . ' cardID: ' . $cardID . ' expiration: ' . $expiration . ' code: ' . $securityCode . '<br>';
        ...
        $update_available_id_table = $mysqli->prepare($query);
        $update_available_id_table->bind_param('ssssssssss', $customerID, $firstname, $lastname, $birthdate, $phone, $email, $street, $city, $state, $zip);
        $stmt->execute(); 
        $update_available_id_table = $
