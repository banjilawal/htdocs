<?php
    ...
    function order_items_query ($order, $customer) {
        ...
        $query = SELECT p.id productID, name, description, grams, retailPrice unitCost, quantity, i.charge cost
        FROM shop.customers c INNER JOIN shop.orders o INNER JOIN shop.orderItems i INNER JOIN shop.products p
        ON i.productID = p.id ON o.id = i.orderID ON c.id = o.customerID 
        WHERE c.id = 'CN-42-QCP-Y2' AND o.id = 'JVXSM-54548-FRKT-628';
