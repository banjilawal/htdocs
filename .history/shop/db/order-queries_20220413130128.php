<?php
    ...
    function order_query ($orderID) {
        ...
        $query = 'SELECT id, submissionDate, estimatedDeliveryDate, actualDeliveryDate, '
            . ' status, customerID FROM orders WHERE id = ?';
    ...
    function order_details_query ($order) {
        ...
        $query = 'SELECT p.id productID, name, description, grams, retailPrice unitCost, quantity, '
            . 'i.charge cost FROM orders o INNER JOIN orderItems i '
            . 'INNER JOIN products p ON i.productID = p.id ON o.id = i.orderID '
            . 'WHERE o.id = ?';
    ...
    function insert_order () {
        ...
        $query = 'SELECT id FROM available_order_ids ORDER BY id LIMIT 1';
