<?php
    ...
    function customer_proteinbar_query ($customer) {
        ...
        $stmt->bind_result($barID, $name, $description, $grams, $retailPrice, $quantity, $deliveryDate, $orderStatus, $cost);
        ...
        while ($stmt->fetch()) {
            ...
            $orderItem = (new OrderItem())->proteinBar($proteinBar)->quantity($quantity)$row = array ($item=> $orderItem, $status=>$orderStatus, $arrival=>$deliveryDate);
        }
        $mysqli->close();
    }   
