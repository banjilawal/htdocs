<?php
    ...
    echo '<p>' . customer_orders_query($customer) .'</p>';

    function customer_order_details ($customer, $order) {
        $customerID = $customer->get_id();
        ...
        $query = 'SELECT p.id, p.name, p.description, p.grams, p.retailPrice unitCost,' i.quantity, i.charge cost
...
WHERE c.id = 'CN-27-QQN-P3' AND o.id = 'GJSEB-15315-UAKU-563'
