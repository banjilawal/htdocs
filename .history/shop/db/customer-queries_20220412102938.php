<?php
    ...
    function insert_credit_card ($customer, $number, $expiration, $code) {
        $mysqli = db_connect();
        $customerID = $customer->get_id();

        $query = 'INSERT INTO shop.cards VALUES (customerID, number, expiration, securityCode, expiration, customerID) VALUES (?, ?, ", ?)';
        $stmt = $mysqli->prepare($query);
        $stmt->bind_param('ssss', $number, $code, $expiration, $customerID);
        $stmt->execute();
    }
