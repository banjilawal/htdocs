<?php
    ...
    function order_details_query ($order) {
        ...
        $query = 'SELECT p.id productID, name, description, grams, retailPrice unitCost, quantity, '
            . 'i.charge cost FROM shop.orders o INNER JOIN shop.orderItems '
            . ' i INNER JOIN shop.products p ON i.productID = p.id ON o.id = i.orderID '
            . ' WHERE o.id = ?';

        $stmt = $mysqli->prepare($query);
        $stmt->bind_param('s', $customerID, $orderID);
        $stmt->execute();
