<?php
    ...
    function customer_proteinbars_query ($customer) {
        $customerID = $customer->get_id();
        $orderBag = new OrderBag();
        $mysqli = db_connect();

        $query = 'SELECT p.id productID, name, description, grams, retailPrice unitCost, quantity, '
            'o.actualDeliveryDate, o.status, i.charge cost 'FROM shop.customers c INNER JOIN shop.orders o INNER JOIN shop.orderItems i
