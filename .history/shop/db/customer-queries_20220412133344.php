<?php
    ...
    function insert_customer () {
       ...
       # echo 'customerID: ' . $customerID . ' cardID: ' . $cardID . ' expiration: ' . $expiration . ' code: ' . $securityCode . '<br>';
        $query = 'INSERT INTO shop.customers VALUES (@firstname, @lastname, @birthdate, @phone, @email, @street, @city, @state, @zip) VALUES (?, ?, ?, ?)';

        $stmt = $mysqli->prepare($query);
        $stmt->bind_param('ssss', $firstname, $lastname, $birthdate, $phone, $email, $street, $city, @state, @zip);
        $stmt->execute();      
    }
