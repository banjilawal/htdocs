<?php
    ...
    function protein_bar_query ($proteinBarID) {
        $mysqli = db_connect();
        $proteinBar = new ProteinBar();

        $query = 'SELECT id, name, description, grams, retailPrice, stars FROM shop.products WHERE id = ?';

        $stmt = $mysqli->prepare($query);
        $stmt->bind_param('s', $proteinBarID);
        $stmt->execute();

        $stmt->bind_result($productID, $name, $description, $grams, $retailPrice, $stars);

        while ($stmt->fetch()) {
            $proteinBar = new ProteinBar();
            $proteinBar->stars($stars);
    ...
    function protein_bar_review_query ($proteinBar) {
        ...
        $query = 'SELECT c.id customerID, firstname, lastname, birthdate, email, phone, street, city, '
            . 'state, zip, r.id reviewID, r.stars, r.comments, r.submissionDate '
            . 'FROM shop.products p INNER JOIN shop.product_reviews r INNER JOIN shop.customers c '
        ...
        while ($stmt->fetch()) {
            ...
            $review->comments($comments)->timestamp(new DateTime ($timestamp));
