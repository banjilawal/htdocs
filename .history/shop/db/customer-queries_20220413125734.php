<?php
    ...
    function customer_query ($customerID) {
        ...
        $query = 'SELECT id, firstname, lastname, birthdate, email, phone, '
            . 'street, city, state, zip FROM customers WHERE id = ?';
    ...
    function customer_order_details_query ($order) {
        ...
        $query = 'SELECT p.id, p.name, p.description, p.grams, p.retailPrice unitCost, '
            ...
            . 'INNER JOIN orderItems i INNER JOIN products p '
    ...
    function customer_proteinbar_query ($customer) {
        ...
        $query = 'SELECT p.id productID, name, description, grams, retailPrice unitCost, quantity, '
            ...
            . 'FROM customers c INNER JOIN orders o INNER JOIN orderItems i '
            . 'INNER JOIN products p ON i.productID = p.id ON o.id = i.orderID ON c.id = o.customerID '
    ...
    function update_phone ($customer, $phone) {
        $customerID = $customer->get_id();
        $mysqli = db_connect();
...
#echo 'from insert-credit-card customerID: ' . $customerID . ' cardID: ' . $cardID . ' expiration: ' . $expiration . ' code: ' . $securityCode . '<br>';
        $query = 'INSERT INTO cards (customerID, id, expiration, securityCode) VALUES (?, ?, ?, ?)';
    ...
    function assign_customer_id () {
        $mysqli = db_connect();
        $result = $mysqli->query('SELECT id FROM available_customer_ids ORDER BY num LIMIT 1');
    ...
    function insert_customer ($firstname, $lastname, $birthdate, $phone, $email, $street, $city, $state, $zip) {
        ...
        $query = 'INSERT INTO customers (id, firstname, lastname, birthdate, phone, email, street, city, state, zip) '
            . 'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
        ...
        $query_b = 'DELETE FROM available_customer_ids WHERE id = ?';
